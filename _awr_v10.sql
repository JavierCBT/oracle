...
begin
    ...
    for v_output in (
		...
		select 'log file sync:           ' ||case
											 when regexp_substr(output, '[0-9\.\,]+us', 1, 4) like '%us'
											 then regexp_substr(output, '[0-9\.\,]+[^ ]*', 1, 4)
											 when to_number(regexp_substr(output, '[0-9\.\,]+', 1, 4)) > 10 or
											      (regexp_substr(output, '[0-9\.\,]+cs', 1, 4) like '%cs' and to_number(regexp_substr(output, '[0-9\.\,]+', 1, 4)) > 1) or
												  (regexp_substr(output, '[0-9\.\,]+s', 1, 4) like '%s' and to_number(regexp_substr(output, '[0-9\.\,]+', 1, 4)) > 0.01)
											 then regexp_substr(output, '[0-9\.\,]+[^ ]*', 1, 4)|| '    *access to disk must be under 10ms' 
											 else regexp_substr(output, '[0-9\.\,]+[^ ]*', 1, 4)
											 end text 
		from awr 
		where output like 'log file sync%' and rownum = 1 union all
