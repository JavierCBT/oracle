...
def SQLPATH=e:\oracle\scripts\tpt -- (Windows)
...
-- some internal variables required for TPT scripts
  ...
  define     3hours="sysdate-1/8 sysdate"
  ...
  define  yesterday="trunc(sysdate-1) sysdate-1"
  define  lastnight="trunc(sysdate-1)+22/24 trunc(sysdate)+10/24"
  define 2nightsago="trunc(sysdate-2)+22/24 trunc(sysdate-1)+10/24"
  define 3nightsago="trunc(sysdate-3)+22/24 trunc(sysdate-2)+10/24"
...
-- format some more columns for common DBA queries
  ...
  col first_seen for a19
  col last_seen for a19
