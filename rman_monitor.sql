...
REM To run from sqlplus:   @monitor '<dd/mm/rr hh24:mi>'
REM Example:  
...
col filename format a125
col bfc  format 9
col "% Compl" format 999.99
...
select s.inst_id "I", CLIENT_INFO ch, context, sofar, totalwork,
...
select s.inst_id "I", CLIENT_INFO Ch, a.STATUS,
open_time, round(BYTES/1024/1024,2) "SOFAR Mb" , round(total_bytes/1024/1024,2)
TotMb, round(BYTES/TOTAL_BYTES*100,2) "% Compl" , a.type, filename
from gv$backup_async_io a,  gv$session s
where not a.STATUS in ('UNKNOWN')
and a.sid=s.sid and open_time > to_date('&1', 'dd/mm/rr hh24:mi') order by 2,7;
...
where
a.sid=s.sid and
open_time > to_date('&1', 'dd/mm/rr hh24:mi') ;
REM -------------------------------